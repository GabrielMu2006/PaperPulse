...
- **订阅驱动的论文发现**：为不同研究方向配置分类、关键词、排除词、机构、期刊/会议与学术来源；每个订阅由纸飞机按钮手动触发推送。
- **可靠的学术筛选**：整合 arXiv、OpenAlex、Crossref 等来源，按标题、DOI、arXiv ID 合并去重，并坚持开放获取与权威性判断。
- **本地优先的资料库**：论文按订阅分组，未归类论文独立管理；同一论文即使属于多个订阅，元数据和 PDF 也只在本机保存一份。
- **面向阅读的工作流**：支持本地 PDF、搜索、收藏、短简介，以及与 PDF 并排显示的异步完整解读；用户调整过的分栏比例会保留。
- **自带模型服务配置**：支持 OpenAI-compatible、Anthropic Messages 与 Gemini GenerateContent 等协议；API Key 仅保存于本机 Keychain。
- **双端独立体验**：当前提供独立的 macOS 与 iOS 应用体验，两端各自保存本地资料与模型配置，不会自动同步。

## 下载与系统要求

### macOS V0.1.1

[下载 PaperPulse-v0.1.1-macOS-arm64.zip](https://github.com/GabrielMu2006/PaperPulse/releases/download/v0.1.1/PaperPulse-v0.1.1-macOS-arm64.zip)
...
- 未配置 API Key 时，生成完整解读会明确提示先在“设置”中配置并保存 API Key，不再误用本地规则摘要作为完整解读。
- 生成完整解读时，简介区域会显示“正在生成完整解读”的进度状态与后台说明；生成期间仍可浏览其他论文。
- 改进 macOS Release 打包：应用 bundle 会在归档前完成 ad-hoc 签名校验，避免下载后被 macOS 误判为损坏。
